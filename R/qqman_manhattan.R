...
#' @param highlight A list of character vector of SNPs in your dataset to highlight. 
#'   These SNPs should all be in your dataset.
#' @param col.highlight A character vector of colors corresponding to the list of "highlight".
...
qqman_manhattan <- function(x, chr="CHR", bp="BP", p="P", snp="SNP", 
                      col=c("gray10", "gray60"), col.highlight="green", chrlabs=NULL,
    ...
    if (!is.null(highlight)) {
        ...
        if (any(!(unlist(highlight) %in% d$SNP))) warning("You're trying to highlight SNPs that don't exist in your results.")
        for( jj in 1:length(highlight)){
			d.highlight=d[which(d$SNP %in% highlight[[jj]]), ]
			with(d.highlight, points(pos, logp, col=col.highlight[jj], pch=20, ...)) 
		}
    }
